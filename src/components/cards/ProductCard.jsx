...
export default function ProductCard({ product }) {
  ...
  const shortDescription = localized(product, 'short_description', i18n.language);
  const categoryName = localized(product.category, 'name', i18n.language);
  const subcategoryName = localized(product.subcategory, 'name', i18n.language);
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 p-3 sm:p-6">
        ...
        <h3 className="text-sm sm:text-2xl font-black text-white leading-tight line-clamp-2">{name}</h3>
        <p className="hidden sm:block text-gray-200 text-sm mt-2 line-clamp-2">{shortDescription}</p>
